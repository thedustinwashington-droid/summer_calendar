...
function applyAccountDefaults(user) {
  if (!user) return;
  document.getElementById('state').value = user.state || 'IN';
  ...
  const selfColor = roleColors[user.role] || '#4b5563';
  const otherColor = user.role === 'father' ? '#ff79b0' : '#2d7ff9';
  const isCustodial = user.custody === 'custodial';
  document.getElementById('custodialName').value = isCustodial ? user.name : 'Custodial Parent';
  document.getElementById('nonCustodialName').value = isCustodial ? 'Non-Custodial Parent' : user.name;
  document.getElementById('custodialColor').value = isCustodial ? selfColor : otherColor;
  document.getElementById('nonCustodialColor').value = isCustodial ? otherColor : selfColor;
...
function renderSavedCalendars() {
  ...
  versions
    ...
    .forEach((version) => {
      ...
      if (selectedVersionId && version.id === selectedVersionId) row.classList.add('active');
  ...
  savedCalendarsEl.appendChild(list);
}
...
loginForm.addEventListener('submit', (event) => {
  ...
  toggleIndianaHolidaySection(user.state === 'IN');
  applyStateGuidance(user.state || 'IN');
  renderSavedCalendars();
});
...
saveVersionBtn.addEventListener('click', () => {
  ...
  const versions = getSavedCalendars(currentUser.name);
  const config = readConfig();
  const label = prompt('Version name', `Version ${versions.length + 1}`)?.trim();
  if (!label) return;
  versions.push({
    id: generateId(),
    label,
...
updateVersionBtn.addEventListener('click', () => {
  ...
  if (scheduleMap.size === 0) {
    alert('Generate a schedule before updating.');
    return;
  }
  const versions = getSavedCalendars(currentUser.name);
  ...
  target.config = readConfig();
  target.schedule = Object.fromEntries(scheduleMap);
  saveSavedCalendars(currentUser.name, versions);
  renderSavedCalendars();
  alert('Saved version updated.');
});
